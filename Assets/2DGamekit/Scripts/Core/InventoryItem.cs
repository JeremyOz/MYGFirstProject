    ...
    public class InventoryItem : MonoBehaviour, IDataPersister
    {
        ...
        void OnTriggerEnter2D(Collider2D other)
        {
            if (layers.Contains(other.gameObject))
            {
                ...
                if (inventoryKey == "Staff")
                    SaveManager.instance.dataSave.MeleeAttackIsActive = true;
                else if (inventoryKey == "Gun")
                    SaveManager.instance.dataSave.RangedAttackIsActive = true;
        ...
        public void LoadingSaveForceAddItem()
        {
            GameObject.Find("Ellen").GetComponent<InventoryController>().AddItem(inventoryKey);
        }
